...
Status: implementation candidate for Harness Specification v1. The harness and a
local LLMBASEDOS evidence run are implemented; publication, independently
reviewed competitor runs, the public post, and market outreach are not yet done.
...
The normative definition of `NOT_REPRESENTABLE` is: “le modèle de données du système n'attache aucune sémantique d'enforcement native à ce concept ; la capacité à stocker un label que le système ne lira jamais ne compte pas comme représentation.” It is not a product-quality verdict. Comparison prose must lead with genuinely comparable attacks and disclose capability gaps below the table.
